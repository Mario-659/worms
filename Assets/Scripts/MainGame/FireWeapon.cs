...
public class FireWeapon : MonoBehaviour
{
    ...
    void Fire()
    {
        Vector3 shootPosition = firePoint.position; // XDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
        if(shootPosition.x > 0)
            shootPosition.x += 0.2f;
        else 
            shootPosition.x -= 0.2f;
        if(shootPosition.y > 0)
            shootPosition.y += 0.4f;
        else
            shootPosition.y += 0.4f;
        // Create a new bullet instance  
        GameObject bullet = Instantiate(bulletPrefab, shootPosition, firePoint.rotation);
